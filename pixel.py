...
def color_clustering(idx, image, k):
    cluster_values = []
    for _ in range(0, k):
...
def segment_img_clr_rgb(image, k):
...
if __name__ == '__main__':
    #  Show image
    IMG_PATH = pixelate(IMG_PATH, 64, 64)
    IMG_PATH = k_means_image(IMG_PATH, 5)
    cv2.cv2.imshow('image', IMG_PATH)
    cv2.cv2.waitKey(0)
